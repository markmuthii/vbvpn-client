...
export class App {
  ...
  constructor(
    ...
    // important-m Custom Event 
    this.rootEl.addEventListener('CustomAddServerEvent', this.customAddServerEvent.bind(this));
    ...
    this.rootEl.addEventListener('LogoutPressed', this.logoutFromApp.bind(this));
    this.rootEl.addEventListener('ShowToast', this.showToastEvent.bind(this));
    this.rootEl.addEventListener('ReloadPage', this.reloadPage.bind(this));
  ...
  // important-m custom function for adding server
  private customAddServerEvent(event: CustomEvent) {
    const serverView = this.rootEl.$.serversView;
    ...
    const serverConfig = this.generateServerConfig(accessKey);

    if(!this.serverRepo.containsServer(serverConfig)){
      try {
        serverView.customSetServerConfig(accessKey, serverConfig);
        // Add to list of servers
        // Causing a sorry, an unexpected error occurred
  ...
  private confirmAddServer(accessKey: string, fromClipboard = false) {
    ...
    const serverConfig = { // important-m
  ...
  // important-m to generate the serverConfig from the access key
  private generateServerConfig(accessKey: string) {
    ...
    const name = this.localize('server-default-name-outline');
    const serverConfig = { // important-m
      host: shadowsocksConfig.host.data,
      port: shadowsocksConfig.port.data,
      method: shadowsocksConfig.method.data,
  ...
  private logoutFromApp(event: CustomEvent) {
    console.log(event);
    ...
    // causing a bug. todo: find out why later
    // moved to hack-friendly space (non-ts)
    // const serverInfo: String = localStorage.getItem("servers") !== null ? localStorage.getItem("servers") : "";
    // const serverId = Object.keys(serverInfo)[0];
    this.rootEl.DEFAULT_PAGE = "login";
    this.rootEl.changePage("login");
    this.rootEl.reloadApp(500);
  }


  // reload the app
  // bound to custom event ReloadPage
  // accepts one parameter, the duration wait time in ms before reloading (timeout)
  private reloadPage(event: CustomEvent) {
  ...
  private showToastEvent(event: CustomEvent) {
    ...
    this.rootEl.showToast(text, duration, buttonText, buttonHandler, buttonUrl);
